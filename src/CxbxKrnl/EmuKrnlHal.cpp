	...
	case ReturnFirmwareQuickReboot:
	{
		...
		else
		{
			...
			// Relaunch Cxbx, to load another Xbe
			{
				...
				snprintf(szArgsBuffer, 4096, "\"%s\" %u %d \"%s\"", XbePath.c_str(), CxbxKrnl_hEmuParent, CxbxKrnl_DebugMode, CxbxKrnl_DebugFileName);
				if ((int)ShellExecute(NULL, "open", szFilePath_CxbxLoader_Exe, szArgsBuffer, szWorkingDirectoy, SW_SHOWDEFAULT) <= 32)

...
//  Debug child processes using https://marketplace.visualstudio.com/items?itemName=GreggMiskelly.MicrosoftChildProcessDebuggingPowerTool
	...
	{
#ifdef _DEBUG_TRACE
		printf("[0x%.4X] INIT: CxbxKrnlInit\n"
			"(\n"
			"   hwndParent          : 0x%.8X\n"
			"   pTLSData            : 0x%.8X\n"
			"   pTLS                : 0x%.8X\n"
	...
	printf("[0x%.4X] INIT: Allocated contiguous page zero at 0x%.8X (to bootstrap NV2A)\n", GetCurrentThreadId(), PageZero);
	...
	printf("[0x%.4X] INIT: Allocated dummy kernel image at 0x%.8X\n", GetCurrentThreadId(), KernelBase);
	...
	// Read which components need to be LLE'ed :
	{
		...
		if (bLLE_APU)
			printf("[0x%.4X] INIT: LLE enabled for APU.\n", GetCurrentThreadId());

		bLLE_GPU = (CxbxLLE_Flags & LLE_GPU) > 0;
		if (bLLE_GPU)
			printf("[0x%.4X] INIT: LLE enabled for GPU.\n", GetCurrentThreadId());

		bLLE_JIT = (CxbxLLE_Flags & LLE_JIT) > 0;
		if (bLLE_JIT)
			printf("[0x%.4X] INIT: LLE enabled for JIT.\n", GetCurrentThreadId());
	}
	...
	// Process XInput Enabled flag
	{
		...
		if (XInputEnabled) {
			...
			printf("[0x%.4X] INIT: Using XInput\n", GetCurrentThreadId());
		} else {
			g_XInputEnabled = false;
			printf("[0x%.4X] INIT: Using DirectInput\n", GetCurrentThreadId());
		}
	}

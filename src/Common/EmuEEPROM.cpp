...
xboxkrnl::XBOX_EEPROM *CxbxRestoreEEPROM(char *szFilePath_EEPROM_bin)
{
	...
	if (NeedsInitialization)
	{
		...
		pEEPROM->UserSettings.VideoFlags = (AV_FLAGS_LETTERBOX | AV_FLAGS_60Hz) >> AV_USER_FLAGS_SHIFT; // = 0x10 (Letterbox) + 0x40 (60 Hz)
		...
		pEEPROM->FactorySettings.AVRegion = AV_STANDARD_NTSC_M >> AV_STANDARD_SHIFT; // = 0x01 = NTSC_M
